...
import { addFilteredOtherReport, addOtherReport, selectFilteredOtherReports, selectOtherReports } from "../../../reducers/reportSlice";
...
const OtherReport = () => {
    ...
    useEffect(() => {
        const getPatients = async () => {
            ...
            if (reportsArray.length > 0) {
                dispatch(addOtherReport(reportsArray));
                setPageLoading(false);
            } else {
                dispatch(addOtherReport([]));
                setPageLoading(false);
            }
        };
    ...
    const otherReports = useSelector(selectOtherReports);
    const filteredOtherReport = useSelector(selectFilteredOtherReports);


    const handleOnSearchChange = () => {
        if (searchText) {
            const text = searchText.toLocaleLowerCase();
            const searchedReport = otherReports.filter((report) => {
                return report?.patientName?.toLocaleLowerCase()?.includes(text);
            });
    
            // Update state with filtered patients
            dispatch(addFilteredOtherReport(searchedReport));
            setFilters(true);
        } else {
            // Update state with filtered patients
            dispatch(addFilteredOtherReport([]));
            setFilters(false);
        }
    };
    ...
    const sortedOtherReports = otherReports
    .slice()
    .sort((a, b) => new Date(b.reportDate) - new Date(a.reportDate))
    .map((report, index) => ({ ...report, key: index + 1 }));


    const sortedFilteredOtherReport = filteredOtherReport
    .slice()
    .sort((a, b) => new Date(b.reportDate) - new Date(a.reportDate))
    .map((report, index) => ({ ...report, key: index + 1 }));
            ...
            <div className="pt-4">
                ...
                <>
                    <div className="pt-4">
                        <Table
                            columns={columns}
                            dataSource={sortedFilteredOtherReport}
                            size="middle"
